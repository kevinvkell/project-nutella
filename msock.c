...
int mrecv(int sock, struct sockaddr_in *info, char *message, int max_len) {
  socklen_t addrlen;
  ...
  addrlen = sizeof(adr[sock]);
  int ret = recvfrom(sock, message, max_len, 0, 
		   (struct sockaddr *) &adr[sock], &addrlen);
  memcpy(info, &adr[sock], sizeof (struct sockaddr_in));
  return ret; 
}
